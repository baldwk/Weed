...
- External plugin UI supports trusted registry install/update plus ZIP/folder import into `%LOCALAPPDATA%\Weed\plugins`.
...
- `plugins.registry.json`, `docs\`, `schemas\`, and `templates\` inside the app folder for plugin development.
...
Plugin SDK contracts live in `Weed.Abstractions`. A manifest schema, plugin registry schema, and C# plugin template are included in the published app folder.

## External Plugin Distribution

External plugins should live in independent repositories and publish ZIP packages through GitHub Releases. The package ZIP root must contain `manifest.json`, the plugin DLL, `.deps.json`, and dependencies. Weed can install trusted plugins from `plugins.registry.json` or import a downloaded ZIP manually from Settings > External Plugins.
